...
export default function BidPage() {
  ...
  const agreedToTermsRef = useRef(null);
  ...
  useEffect(() => {
    if (data) {
      const timer = setInterval(() => {
        setTimeLeft(calculateRemainTime(data.endAuctionTime));
      }, 1000);
  ...
  useEffect(() => {
    if (data) {
      setNewPrice(data.currentPrice + 1000);
    }
  }, [data]);
  ...
  function submitBidPrice(id, price, userToken) {
    if (!agreedToTerms) {
      alert('약관에 동의해주세요.');
      agreedToTermsRef.current.focus();
      return;
    }
    if (newPrice < data.currentPrice + 1000) {
      alert('최소 입찰가보다 낮습니다.');
        ...
        <div className="w-full p-6 lg:w-[1024px]">
          ...
          <div className="terms-agreement overflow-y-auto h-40 my-5 p-2 border border-grayish rounded">
            <p className="text-xs text-left whitespace-normal">
              ...
              증가액 등을 포함합니다.
              <br />
              정보의 정확성: 참여자는 제공하는 모든 정보가 정확하고 최신의
              것임을 보장합니다. 부정확한 정보 제공으로 인한 책임은 참여자에게
              있습니다.
              <br />
              물품의 상태 확인: 참여자는 경매에 부쳐진 물품의 상태와 설명을 주의
              깊게 검토하고 이에 대한 책임은 참여자 본인에게 있음을 인지합니다.
              <br />
              결제 조건: 낙찰된 참여자는 경매 약관에 명시된 기간 내에 전체
              결제를 완료해야 합니다. 지불 지연은 추가 수수료 또는 경매 참여
              자격 상실을 초래할 수 있습니다.
              <br />
              책임의 한계: 경매 주최자는 경매 물품의 품질, 상태 또는 설명의
              정확성에 대해 책임을 지지 않습니다. 모든 물품은 {`'있는 그대로'`}
              의 상태로 제공되며, 모든 판매는 최종적입니다.
              <br />
            </p>
          </div>
          ...
          <div className="flex justify-center space-x-2 my-5">
            ...
            <TERipple rippleColor="white">
              <button
                type="button"
                className="inline-block rounded bg-deepblue1 hover:bg-deepblue px-6 pb-2 pt-2.5 text-xs font-medium uppercase leading-normal text-white shadow-[0_4px_9px_-4px_#3b71ca] transition duration-150 ease-in-out hover:bg-deepblue2 hover:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:bg-primary-600 focus:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:outline-none focus:ring-0 active:bg-primary-700 active:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] dark:shadow-[0_4px_9px_-4px_rgba(59,113,202,0.5)] dark:hover:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.2),0_4px_18px_0_rgba(59,113,202,0.1)] dark:focus:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.2),0_4px_18px_0_rgba(59,113,202,0.1)] dark:active:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.2),0_4px_18px_0_rgba(59,113,202,0.1)]"
                disabled={loading}
                onClick={() => {
                  submitBidPrice(auctionId, newPrice, accessToken);
                }}
              >
                상품 입찰하기
              </button>
