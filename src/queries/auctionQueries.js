...
export async function createAuction() {
  await axios.post(`${API_SERVER}/api/auction`);
}

// 즉시 결제
export async function purchaseInstant(auctionId) {
  await axios.post(`${API_SERVER}/api/auction/${auctionId}/instant`);
}

// '상품 입찰하기' 페이지 구성에 필요한 데이터 받아오기
export async function fetchBidPage(auctionId) {
  const { data } = await axios.get(
    `${API_SERVER}/api/auction/${auctionId}/bid`,
  );
  return data;
}
...
export async function bid(auctionId, price) {
  await axios.post(`${API_SERVER}/api/auction/${auctionId}/bid`, { price });
}
...
export async function fetchProductInfo(auctionId) {
  ...
  try {
    const response = await axios.get(`${API_SERVER}/api/auction/${auctionId}`, {
...
export async function deleteAuction(auctionId) {
  await axios.delete(`${API_SERVER}/api/auction/${auctionId}`);
}

// 판매자 정보 조회
export async function fetchSellerInfo(auctionId) {
  const { data } = await axios.get(
    `${API_SERVER}/api/auction/${auctionId}/seller`,
  );
  return data;
}
